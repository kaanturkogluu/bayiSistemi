...
@section('content')
    ...
    <div x-data="maintenanceForm()" class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <form action="{{ route('admin.maintenances.store') }}" method="POST" class="p-6 sm:p-8" @submit="isSubmitting = true">
            ...
            <!-- Single Line Layout for Main Info -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 pb-8 border-b border-slate-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-6">
                    <div>
                        <label for="customer_id" class="block text-sm font-medium text-slate-700 mb-1">Müşteri Seçimi <span class="text-red-500">*</span></label>
                        <div class="relative" x-init="$(function(){ $($refs.customerSelect).select2({placeholder: 'Müşteri Ara veya Seç...', allowClear: true, width: '100%', theme: 'classic'}).on('change', function(e){ selectedCustomer = e.target.value; }); if(selectedCustomer) $($refs.customerSelect).val(selectedCustomer).trigger('change.select2'); })">
                    ...
                    <div x-show="availableVehicles.length > 0">
                        <label for="vehicle_id" class="block text-sm font-medium text-slate-700 mb-1">Araç Plakası <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <select id="vehicle_id" name="vehicle_id" :required="availableVehicles.length > 0" class="w-full pl-4 pr-10 py-3 rounded-lg border @error('vehicle_id') border-red-300 focus:ring-red-500 focus:border-red-500 @else border-slate-300 focus:ring-blue-500 focus:border-blue-500 @enderror outline-none transition-all bg-white appearance-none uppercase">
                        ...
                        <div class="mt-1 text-xs text-slate-500">Müşteriyi seçtiğinizde kayıtlı araçları listelenecektir.</div>
                    </div>
                </div>
                <!-- Labor Cost -->
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="labor_cost_display" x-mask:dynamic="$money($input, ',', '.', 2)" x-model="displayLaborCost" @input="updateLaborCost($event.target.value)" required
                            class="pl-8 w-full px-4 py-3 rounded-lg border @error('labor_cost') border-red-300 focus:ring-red-500 focus:border-red-500 @else border-slate-300 focus:ring-blue-500 focus:border-blue-500 @enderror outline-none transition-all text-right font-medium"
                            placeholder="0,00">
                        <!-- Hidden input to send actual numeric value to backend -->
                        <input type="hidden" name="labor_cost" :value="laborCost">
                    </div>
                     <p class="mt-1 text-xs text-slate-500">Bakım için alınacak sabit işçilik ücreti.</p>
            ...
            <div class="bg-slate-50 border border-slate-200 rounded-xl p-1 mb-8">
                <!-- Titles for desktop -->
                <div class="hidden md:grid md:grid-cols-12 gap-4 px-4 py-2 bg-slate-100/50 rounded-t-lg border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase">
                    ...
                    <div class="md:col-span-2">Birim Fiyat (TL) <span class="text-red-500">*</span></div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 border-b border-slate-200 last:border-0 items-start md:items-center relative bg-white first:rounded-t-none last:rounded-b-lg group">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="flex items-center">
                                <input type="number" step="1" min="1" x-model.number="part.quantity" :name="'parts['+index+'][quantity]'" required
                                    class="w-full px-3 py-2 text-sm rounded-md border border-slate-300 focus:ring-blue-500 focus:border-blue-500 outline-none text-right font-medium">
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-3">
                            ...
                            <input type="text" x-model="part.note" :name="'parts['+index+'][note]'"
                                class="w-full px-3 py-2 text-sm rounded-md border border-transparent hover:border-slate-300 focus:border-slate-300 bg-slate-50 focus:bg-white focus:ring-blue-500 outline-none transition-all" placeholder="Opsiyonel not yazabilirsiniz...">
                        </div>
            ...
            <div class="pt-4 flex items-center justify-end space-x-3 border-t border-slate-100">
                <a href="{{ route('admin.maintenances.index') }}" class="px-5 py-2.5 border border-slate-300 rounded-lg text-slate-700 bg-white hover:bg-slate-50 font-medium transition-colors">
                    İptal
                </a>
                <button type="submit" 
                        :disabled="isSubmitting" 
                        class="px-5 py-2.5 border border-transparent rounded-lg text-white font-medium transition-colors shadow-sm text-lg flex items-center"
                        :class="isSubmitting ? 'bg-slate-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'">
                    <span x-show="!isSubmitting">Bakımı Kaydet</span>
    ...
    <script>
        ...
        // Custom Alpine x-init handles Select2 now for better binding
    </script>
