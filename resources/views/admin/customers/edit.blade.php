...
@section('header', 'Müşteri Düzenle')
...
@section('content')
    <div class="mb-6 flex items-center">
        ...
        <h1 class="text-2xl font-bold text-slate-800">Müşteriyi Düzenle: {{ $customer->name_surname }}</h1>
    </div>

    <div x-data="customerForm()" class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-2xl">
        <form action="{{ route('admin.customers.update', $customer) }}" method="POST" class="p-6 sm:p-8 space-y-6" @submit="isSubmitting = true">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="name_surname" id="name_surname" value="{{ old('name_surname', $customer->name_surname) }}" required
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="tc_no" id="tc_no" value="{{ old('tc_no', $customer->tc_no) }}" maxlength="11" x-mask="99999999999"
            ...
            <!-- Phone -->
            <div x-data="{ phoneStr: '{{ str_replace('+90 ', '', str_replace('+90', '', old('phone', $customer->phone))) }}' }">
            ...
            <div>
                ...
                <textarea name="address" id="address" rows="3"
                    ...
                    placeholder="Müşterinin açık adresi...">{{ old('address', $customer->address) }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
                ...
                plates: [
                    @if(old('plates'))
                        @foreach(old('plates') as $plate)
                            { value: "{{ $plate }}" },
                        @endforeach
                    @elseif(isset($customer) && $customer->vehicles->count() > 0)
