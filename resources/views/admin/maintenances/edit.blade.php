...
@section('header', 'Bakım Kaydını Düzenle')
...
@section('content')
    <div class="mb-6 flex items-center">
        ...
        <h1 class="text-2xl font-bold text-slate-800">Bakım Kaydını Düzenle: #{{ $maintenance->id }}</h1>
    </div>
    ...
    <div x-data="maintenanceForm()" class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <form action="{{ route('admin.maintenances.update', $maintenance) }}" method="POST" class="p-6 sm:p-8" @submit="isSubmitting = true">
            @csrf
            @method('PUT')
                ...
                <div class="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-6 font-semibold flex flex-col md:flex-row gap-6">
                    <div class="flex-1">
                        ...
                        <div class="relative w-full">
                           <input type="hidden" id="hidden_km" name="km" value="{{ old('km', $maintenance->km) }}">
                           <input type="text" id="km_display" value="{{ old('km', $maintenance->km) !== null && old('km', $maintenance->km) !== '' ? number_format((float)old('km', $maintenance->km), 0, '', '.') : '' }}"
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('maintenanceForm', () => ({
                isSubmitting: false,
                laborCost: {{ old('labor_cost', $maintenance->labor_cost) }},
                displayLaborCost: '{{ number_format(old('labor_cost', $maintenance->labor_cost), 2, ',', '.') }}',
                selectedCustomer: '{{ old('customer_id', $maintenance->customer_id) }}',
                ...
                parts: [
                    @if(old('parts'))
                        ...
                        @endforeach
                    @elseif($maintenance->parts->count() > 0)
                        @foreach($maintenance->parts as $part)
                            { 
                                name: "{{ $part->name }}", 
                                quantity: {{ $part->quantity }}, 
                                unit_price: {{ $part->unit_price }},
                                display_unit_price: '{{ number_format($part->unit_price, 2, ',', '.') }}',
                                note: "{{ $part->note ?? '' }}" 
                            },
                        @endforeach
                    @else
